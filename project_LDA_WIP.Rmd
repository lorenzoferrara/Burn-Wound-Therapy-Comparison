...
plot1 = ggplot(df1, aes(x = burn.type, y = counts)) +
  geom_bar(fill = c("green3", "royalblue3", "yellow2", "red3"), stat = "identity") # +
...
plot2 = ggplot(df2, aes(x = gender, y = counts)) +
  geom_bar(fill = c("royalblue3", "salmon2"), stat = "identity") # + 
  # geom_text(aes(label = counts), vjust = -0.3)
...
This representation as a proportional odds model facilitates the interpretation of the model's results.
...
The Log-logistic model allows us to analyze the Odds Ratio and Acceleration Factor of the various covariates in relation to the baseline. The Odds Ratio represents the ratio of the odds of an event occurring in one group to the odds of the event occurring in another group. 
...
![Table 5](images/Table5.png){width=70%}
...
This value can be find in [Table 5]() under the column of "exp(coef)".
...
In this report, we analyzed the survival times of burn patients using non-parametric and semi-parametric models. Our results indicate that the "body cleansing" treatment and being female are associated with longer survival times, while burns on the buttock and electrical burns are associated with shorter survival times. Our models also pass the assumption of proportional hazards, and we conclude that the Log-logistic model is the optimal one to describe the phenomenon.

In this study, we analyzed data from burn patients who were treated with either standard bathing care or body cleansing treatment. We used non-parametric and semi-parametric methods to analyze the survival times and hazard ratios, respectively. Our results indicate that the body cleansing treatment is more effective in increasing survival times, with a hazard ratio of 0.48 compared to the standard treatment. We also found that women have a lower hazard ratio than men, and that burns on the buttock are a significant risk factor. Additionally, we found that electrical burns have a much higher hazard ratio compared to other types of burns, making them the most dangerous. We also found that a Log-logistic model is the best model to describe our phenomenon, and that the residuals of the model are close to the theoretical ones. We also found that the assumption of proportional hazards is reasonable for this study.
