...
ggplot(df, aes(x = gender, y = counts)) +
  geom_bar(fill = c("#0073C2FF", "sienna1"), stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3)
```
...
## With these data, a stratified test might be a good idea, because of the unbalanced deisgn of the experiment and because our ultime goal is the one to verify the effect of each treatment to intervene in a specific medical situation (with specific burn locations and percentage of body burnt)
